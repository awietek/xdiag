...
Op combine_matrix_ops(std::vector<Op> const &ops, int64_t d) try {
  ...
  for (Op const &op : ops) {
    if (op.type() != "Matrix") {
      XDIAG_THROW(
          fmt::format("combine_matrix_ops: all ops must be of type \"Matrix\", "
                      "got \"{}\"",
                      op.type()));
    }
    operators::must_have_sites(op);
    operators::must_have_matrix(op);
  }
  ...
  if (all_real) {
    ...
    for (std::size_t i = 1; i < ops.size(); ++i) {
      combined = combined * embed_op<double>(ops[i], all_sites, site_to_pos, d);
    }
