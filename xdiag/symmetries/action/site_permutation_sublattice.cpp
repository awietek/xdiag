#include "site_permutation_sublattice.hpp"

#include <limits>

#include <xdiag/symmetries/action/site_permutation.hpp>
#include <xdiag/symmetries/action/sublattice_stability.hpp>
#include <xdiag/utils/logger.hpp>

namespace xdiag::symmetries {

template <typename bit_t, int n_sublat>
SitePermutationSublattice<bit_t, n_sublat>::SitePermutationSublattice(
  ...
  if (!symmetries::is_sublattice_stable(n_sublat, group)) {
    Log.err("Error creating SitePermutationSublattice with {} sublattices: "
            "permutation group is not {}-sublattice stable!",
            n_sublat, n_sublat);
  }

  int64_t n_trailing = (n_sublat - 1) * nsites_sublat_;
  auto group_action = SitePermutation(permutation_group_);
...
template <typename bit_t, int n_sublat>
bit_t SitePermutationSublattice<bit_t, n_sublat>::apply(int64_t sym,
                                                        bit_t state) const {
  bit_t translated = 0;
  for (int sublat = 0; sublat < n_sublat; ++sublat) {
...
template <typename bit_t, int n_sublat>
bit_t SitePermutationSublattice<bit_t, n_sublat>::representative(
    bit_t state) const {
...
SitePermutationSublattice<bit_t, n_sublat>::representative_sym(
    bit_t state) const {
...
SitePermutationSublattice<bit_t, n_sublat>::representative_syms(
    bit_t state) const {
...
template <typename bit_t, int n_sublat>
bool SitePermutationSublattice<bit_t, n_sublat>::operator==(
    SitePermutationSublattice const &rhs) const {
