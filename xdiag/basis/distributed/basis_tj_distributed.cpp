...
BasistJDistributed<bit_t>::BasistJDistributed(int64_t nsites, int64_t nup, int64_t ndn) try
...
template <typename bit_t> int64_t BasistJDistributed<bit_t>::nsites() const {
  return nsites_;
}
template <typename bit_t> int64_t BasistJDistributed<bit_t>::nup() const { return nup_; }

template <typename bit_t> int64_t BasistJDistributed<bit_t>::ndn() const { return ndn_; }

template <typename bit_t>
int64_t BasistJDistributed<bit_t>::index(bit_t up, bit_t dn) const {
...
template <typename bit_t> int64_t BasistJDistributed<bit_t>::dim() const { return dim_; }
template <typename bit_t> int64_t BasistJDistributed<bit_t>::size() const { return size_; }
template <typename bit_t> int64_t BasistJDistributed<bit_t>::size_transpose() const {
  return size_transpose_;
}
template <typename bit_t> int64_t BasistJDistributed<bit_t>::size_max() const {
  return size_max_;
}
template <typename bit_t> int64_t BasistJDistributed<bit_t>::size_min() const {
  return size_min_;
}
template <typename bit_t>
typename BasistJDistributed<bit_t>::iterator_t BasistJDistributed<bit_t>::begin() const {
  return iterator_t(*this, true);
}
template <typename bit_t>
typename BasistJDistributed<bit_t>::iterator_t BasistJDistributed<bit_t>::end() const {
  return iterator_t(*this, false);
}

template <typename bit_t>
std::vector<bit_t> const &BasistJDistributed<bit_t>::my_ups() const {
  return my_ups_;
}

template <typename bit_t>
int64_t BasistJDistributed<bit_t>::my_ups_offset(bit_t ups) const {
  return my_ups_offset_.at(ups);
}

template <typename bit_t>
gsl::span<bit_t> BasistJDistributed<bit_t>::my_dns_for_ups(int64_t idx_ups) const {
  return my_dns_for_ups_[idx_ups];
}

template <typename bit_t>
std::vector<bit_t> const &BasistJDistributed<bit_t>::my_dns() const {
  return my_dns_;
}
template <typename bit_t>
int64_t BasistJDistributed<bit_t>::my_dns_offset(bit_t dns) const {
  return my_dns_offset_.at(dns);
}
template <typename bit_t>
gsl::span<bit_t> BasistJDistributed<bit_t>::my_ups_for_dns(int64_t idx_dns) const {
  return my_ups_for_dns_[idx_dns];
}

template <typename bit_t>
template <typename coeff_t>
void BasistJDistributed<bit_t>::transpose(const coeff_t *in_vec, coeff_t *out_vec) const
    try {
...
template void BasistJDistributed<uint64_t>::transpose(const complex *, complex *) const;

template <typename bit_t>
template <typename coeff_t>
void BasistJDistributed<bit_t>::transpose_r(coeff_t const *in_vec, coeff_t *out_vec) const
    try {
...
template void BasistJDistributed<uint64_t>::transpose_r(const complex *, complex *) const;

template class BasistJDistributed<uint32_t>;
template class BasistJDistributed<uint64_t>;
...
template <typename bit_t>
BasistJDistributedIterator<bit_t> &BasistJDistributedIterator<bit_t>::operator++() {
  ++dn_idx_;
...
template <typename bit_t>
std::pair<bit_t, bit_t> BasistJDistributedIterator<bit_t>::operator*() const {
...
template <typename bit_t>
bool BasistJDistributedIterator<bit_t>::operator!=(
    BasistJDistributedIterator<bit_t> const &rhs) const {
