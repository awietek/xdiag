...
BasisElectronDistributed<bit_t>::BasisElectronDistributed(int64_t nsites,
                                                          int64_t nup,
                                                          int64_t ndn) try
...
template <typename bit_t>
int64_t BasisElectronDistributed<bit_t>::nsites() const {
  return nsites_;
}
template <typename bit_t> int64_t BasisElectronDistributed<bit_t>::nup() const {
  return nup_;
}

template <typename bit_t> int64_t BasisElectronDistributed<bit_t>::ndn() const {
  return ndn_;
}

template <typename bit_t>
int64_t BasisElectronDistributed<bit_t>::index(bit_t up, bit_t dn) const {
...
template <typename bit_t>
int64_t BasisElectronDistributed<bit_t>::size() const {
  return size_;
}
template <typename bit_t>
int64_t BasisElectronDistributed<bit_t>::size_transpose() const {
  return size_transpose_;
}
template <typename bit_t>
int64_t BasisElectronDistributed<bit_t>::size_max() const {
  return size_max_;
}
template <typename bit_t>
int64_t BasisElectronDistributed<bit_t>::size_min() const {
  return size_min_;
}
template <typename bit_t>
typename BasisElectronDistributed<bit_t>::iterator_t
BasisElectronDistributed<bit_t>::begin() const {
  return iterator_t(*this, true);
}
template <typename bit_t>
typename BasisElectronDistributed<bit_t>::iterator_t
BasisElectronDistributed<bit_t>::end() const {
  return iterator_t(*this, false);
}

template <typename bit_t>
std::vector<bit_t> const &BasisElectronDistributed<bit_t>::my_ups() const {
  return my_ups_;
}

template <typename bit_t>
int64_t BasisElectronDistributed<bit_t>::my_ups_offset(bit_t ups) const {
  return my_ups_offset_.at(ups);
}

template <typename bit_t>
std::vector<bit_t> const &BasisElectronDistributed<bit_t>::all_dns() const {
  return all_dns_;
}

template <typename bit_t>
std::vector<bit_t> const &BasisElectronDistributed<bit_t>::my_dns() const {
  return my_dns_;
}
template <typename bit_t>
int64_t BasisElectronDistributed<bit_t>::my_dns_offset(bit_t dns) const {
  return my_dns_offset_.at(dns);
}
template <typename bit_t>
std::vector<bit_t> const &BasisElectronDistributed<bit_t>::all_ups() const {
  return all_ups_;
}

template <typename bit_t>
template <typename coeff_t>
void BasisElectronDistributed<bit_t>::transpose(const coeff_t *in_vec,
                                                coeff_t *out_vec) const try {
...
template void BasisElectronDistributed<uint32_t>::transpose(const complex *,
                                                            complex *) const;
template void BasisElectronDistributed<uint64_t>::transpose(const double *,
                                                            double *) const;
template void BasisElectronDistributed<uint64_t>::transpose(const complex *,
                                                            complex *) const;

template <typename bit_t>
template <typename coeff_t>
void BasisElectronDistributed<bit_t>::transpose_r(coeff_t const *in_vec,
                                                  coeff_t *out_vec) const try {
  // transforms a vector in up/dn order to dn/up order
  // result of transpose is stored in send_buffer
...
template void BasisElectronDistributed<uint32_t>::transpose_r(const complex *,
                                                              complex *) const;
template void BasisElectronDistributed<uint64_t>::transpose_r(const double *,
                                                              double *) const;
template void BasisElectronDistributed<uint64_t>::transpose_r(const complex *,
                                                              complex *) const;

template class BasisElectronDistributed<uint32_t>;
template class BasisElectronDistributed<uint64_t>;

template <typename bit_t>
BasisElectronDistributedIterator<bit_t>::BasisElectronDistributedIterator(
    BasisElectronDistributed<bit_t> const &basis, bool begin)
    : basis_(basis), up_idx_(begin ? 0 : basis_.my_ups().size()), dn_idx_(0) {}

template <typename bit_t>
BasisElectronDistributedIterator<bit_t> &
...
BasisElectronDistributedIterator<bit_t>::operator*() const {
...
template <typename bit_t>
bool BasisElectronDistributedIterator<bit_t>::operator!=(
    BasisElectronDistributedIterator<bit_t> const &rhs) const {
