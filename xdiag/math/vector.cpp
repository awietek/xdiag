...
template <> arma::vec Vector::as<arma::vec>() const try {
  if (const arma::vec *m = std::get_if<arma::vec>(&vec_)) {
    ...
    XDIAG_THROW(fmt::format("Cannot convert Vector holding a value of type "
                            "\"{}\" to value of type \"{}\"",
                            utils::get_type_name<arma::cx_vec>(),
                            utils::get_type_name<arma::vec>()));
  }
}
XDIAG_CATCH
...
arma::vec Vector::real() const {
  return std::visit(utils::overload{[](arma::vec const &v) { return v; },
                                    [](arma::cx_vec const &v) {
                                      return arma::vec(arma::real(v));
                                    }},
                    vec_);
}

arma::vec Vector::imag() const {
  return std::visit(utils::overload{[](arma::vec const &v) {
                                      return arma::vec(v.n_elem,
                                                       arma::fill::zeros);
                                    },
                                    [](arma::cx_vec const &v) {
                                      return arma::vec(arma::imag(v));
                                    }},
                    vec_);
}
Vector Vector::conj() const {
  return std::visit(
      utils::overload{
          [](arma::vec const &v) { return Vector(v); },
          [](arma::cx_vec const &v) { return Vector(arma::conj(v)); }},
      vec_);
}

Vector Vector::to_real(double tol) const try {
  if (isreal()) {
    return *this;
  }
  arma::cx_vec const &v = std::get<arma::cx_vec>(vec_);
  if (arma::norm(arma::imag(v), "inf") > tol) {
    XDIAG_THROW(
        fmt::format("Cannot narrow Vector to real: max imaginary component {} "
                    "exceeds tolerance {}",
                    arma::norm(arma::imag(v), "inf"), tol));
  }
  return Vector(arma::vec(arma::real(v)));
}
XDIAG_CATCH
...
Vector &Vector::operator*=(Scalar const &rhs) {
  if (rhs.isreal()) {
    std::visit(utils::overload{[&](arma::vec &m) {
                                 vec_ = arma::vec(m * rhs.as<double>());
                               },
                               [&](arma::cx_vec &m) {
    ...
    std::visit(utils::overload{[&](arma::vec &v) {
...
Vector Vector::operator-() const {
  return std::visit(
      utils::overload{
          [](arma::vec const &a) { return Vector(arma::vec(-a)); },
...
bool isreal(Vector const &v) { return v.isreal(); }
arma::vec real(Vector const &v) { return v.real(); }
arma::vec imag(Vector const &v) { return v.imag(); }
Vector conj(Vector const &v) { return v.conj(); }
