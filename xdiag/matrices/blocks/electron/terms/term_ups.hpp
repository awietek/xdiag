...
// configuration is a spectator, but under symmetry the dn must be re-symmetrised
// in the OUTPUT up representative's block. Outer loop over input up
// representatives; the output up representative and its up-stabilizer are found
// once per input up (hoisted out of the inner dn loop). The matrix element
      ...
      if (basis_out.stab_size(idx_up_out) == 1) {
        ...
        for (bit_t dns : dnss_in) {
          ...
          if (idx_dns_out >= 0) {
            ...
            coeff_t val = coeff * bloch(sym) * norms_out[idx_dns_out] /
                          norms_in[idx_dn];
            XDIAG_FILL(off_in + idx_dn, off_out + idx_dns_out,
                       (fermi_up ^ fermi_dn) ? -val : val);
          }
