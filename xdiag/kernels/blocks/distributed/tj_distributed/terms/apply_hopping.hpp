...
#include <xdiag/kernels/blocks/distributed/tj_distributed/terms/generic_term_dns.hpp>
#include <xdiag/kernels/blocks/distributed/tj_distributed/terms/generic_term_ups.hpp>
...
void apply_hopping(Coeff const &cpl, Op const &op, basis_t const &basis,
  ...
  // kernels/blocks/{electron,tj}). Plain Hop{up,dn} applies the same t in both
  // hop directions; Hop{up,dn}Asym = -Cdag{s1}C{s2} + Cdag{s2}C{s1} flips the
  // sign of the post-flip s2->s1 branch (mirrors electron::term_hop*_asym).
  if (is_asym) {
