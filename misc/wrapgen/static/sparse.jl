...
export coo_matrix, coo_matrix_32, csr_matrix, csr_matrix_32,
       csc_matrix, csc_matrix_32, to_dense
...
function _coo_matrix(ops::OpSum, block_in::Block, ::Type{Ti}; i0=1) where {Ti<:Integer}
    ...
    GC.@preserve row col data begin
        fun_coo_fill(ops.cxx_object, block_in.cxx_object, Int64(nnz),
                     pointer(row), pointer(col), pointer(data), Ti(i0))
    end
...
function _csr_matrix(ops::OpSum, block_in::Block, ::Type{Ti}; i0=1) where {Ti<:Integer}
    ...
    rowptr, col, data = Vector{Ti}(undef, nrows + 1), Vector{Ti}(undef, nnz), Vector{Tc}(undef, nnz)
    GC.@preserve rowptr col data begin
        fun_csr_fill(ops.cxx_object, block_in.cxx_object, counts,
                     pointer(rowptr), pointer(col), pointer(data), Ti(i0), false)
    end
...
function _csc_matrix(ops::OpSum, block_in::Block, ::Type{Ti}; i0=1) where {Ti<:Integer}
    ...
    nrows = Ti(fun_sparse_dim_out(ops.cxx_object, block_in.cxx_object))
    colptr, row, data = Vector{Ti}(undef, ncols + 1), Vector{Ti}(undef, nnz), Vector{Tc}(undef, nnz)
    GC.@preserve colptr row data begin
        fun_csr_fill(ops.cxx_object, block_in.cxx_object, counts,
                     pointer(colptr), pointer(row), pointer(data), Ti(i0), true)
    end
...
function apply(A::CSRMatrix, x::AbstractVector, y::AbstractVector)
    ...
    GC.@preserve A xin y fun_csr_apply_vec(
        csr_view(A), pointer(xin), pointer(y), length(xin), length(y))
    return y
end
...
function apply(A::CSRMatrix, x::AbstractMatrix, y::AbstractMatrix)
    ...
    GC.@preserve A xin y fun_csr_apply_mat(
        csr_view(A), pointer(xin), pointer(y), size(xin, 1), size(xin, 2), size(y, 1))
    return y
end
...
to_dense(A::COOMatrix) = GC.@preserve A to_julia(fun_coo_to_dense(
    A.nrows, A.ncols, pointer(A.row), pointer(A.col), pointer(A.data), length(A.data), A.i0))
to_dense(A::CSCMatrix) = GC.@preserve A to_julia(fun_csc_to_dense(
    A.nrows, A.ncols, pointer(A.colptr), pointer(A.row), pointer(A.data), length(A.data), A.i0))
