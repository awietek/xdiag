...
//! (taken from all the possible typedefs of {u8, s8, ..., u64, s64} and the
//! other native types. We can't use the actual u8/s8 typedefs because their
//! relations to the H5T_... types are unclear.
template <> inline hid_t get_hdf5_type<unsigned char>() {
  return arma_H5Tcopy(arma_H5T_NATIVE_UCHAR);
}

template <> inline hid_t get_hdf5_type<char>() {
  return arma_H5Tcopy(arma_H5T_NATIVE_CHAR);
}

template <> inline hid_t get_hdf5_type<short>() {
  return arma_H5Tcopy(arma_H5T_NATIVE_SHORT);
}

template <> inline hid_t get_hdf5_type<unsigned short>() {
  return arma_H5Tcopy(arma_H5T_NATIVE_USHORT);
}

template <> inline hid_t get_hdf5_type<int>() {
  return arma_H5Tcopy(arma_H5T_NATIVE_INT);
}

template <> inline hid_t get_hdf5_type<unsigned int>() {
  return arma_H5Tcopy(arma_H5T_NATIVE_UINT);
}

template <> inline hid_t get_hdf5_type<long>() {
  return arma_H5Tcopy(arma_H5T_NATIVE_LONG);
}

template <> inline hid_t get_hdf5_type<unsigned long>() {
  return arma_H5Tcopy(arma_H5T_NATIVE_ULONG);
}

template <> inline hid_t get_hdf5_type<long long>() {
  return arma_H5Tcopy(arma_H5T_NATIVE_LLONG);
}

template <> inline hid_t get_hdf5_type<unsigned long long>() {
  return arma_H5Tcopy(arma_H5T_NATIVE_ULLONG);
}

template <> inline hid_t get_hdf5_type<float>() {
  return arma_H5Tcopy(arma_H5T_NATIVE_FLOAT);
}

template <> inline hid_t get_hdf5_type<double>() {
...
template <> inline hid_t get_hdf5_type<std::complex<float>>() {
  ...
  arma_H5Tinsert(type, "real", HOFFSET(hdf5_complex_t<float>, real),
                 arma_H5T_NATIVE_FLOAT);
  arma_H5Tinsert(type, "imag", HOFFSET(hdf5_complex_t<float>, imag),
                 arma_H5T_NATIVE_FLOAT);
...
template <> inline hid_t get_hdf5_type<std::complex<double>>() {
  ...
  arma_H5Tinsert(type, "real", HOFFSET(hdf5_complex_t<double>, real),
                 arma_H5T_NATIVE_DOUBLE);
  arma_H5Tinsert(type, "imag", HOFFSET(hdf5_complex_t<double>, imag),
                 arma_H5T_NATIVE_DOUBLE);
...
inline bool is_supported_arma_hdf5_type(hid_t datatype) {
  hid_t search_type;

  bool is_equal;

  // start with most likely used types: double, complex<double>, float,
  // complex<float>
  ...
  // remaining supported types: u8, s8, u16, s16, u32, s32, u64, s64, ulng_t,
  // slng_t
...
inline herr_t hdf5_search_callback(hid_t loc_id, const char *name,
  ...
  if (info->type == H5O_TYPE_DATASET) {
    ...
    for (size_t string_pos = 0; string_pos < search_info->best_match_position;
         ++string_pos) {
      ...
      // and then simply take the last substring of name containing that number
      // of slashes, we can do the comparison.

      // Count the number of forward slashes in names[string_pos].
      uword name_count = 0;
      for (uword i = 0; i < search_info->names[string_pos].length(); ++i) {
        if ((search_info->names[string_pos])[i] == '/') {
          ++name_count;
        }
      }
      ...
      if (count <= name_count) {
        ...
        if (substring == search_info->names[string_pos]) {
          ...
          // arma_check(match_candidate < 0, "Mat::load(): cannot open an HDF5
          // dataset");
          if (match_candidate < 0) {
            return -1;
          }
      ...
      // If they are not the same, but we have not found anything and we don't
      // need an exact match, take this.
...
//! If 'exact' is true, failure to find a dataset in the list of names means
//! that -1 is returned. If 'exact' is false and no datasets are found, -1 is
//! returned. The number of dimensions is used to help prune down invalid
//! datasets; 2 dimensions is a matrix, 1 dimension is a vector, and 3
//! dimensions is a cube. If the number of dimensions in a dataset is less than
//! or equal to num_dims, it will be considered -- for instance, a
//! one-dimensional HDF5 vector can be loaded as a single-column matrix.
inline hid_t search_hdf5_file(const std::vector<std::string> &names,
                              hid_t hdf5_file, int num_dims = 2,
                              bool exact = false) {
  ...
  herr_t status =
      arma_H5Ovisit(hdf5_file, H5_INDEX_NAME, H5_ITER_NATIVE,
                    hdf5_search_callback, void_ptr(&search_info), H5O_INFO_ALL);

#else
  herr_t status = arma_H5Ovisit(hdf5_file, H5_INDEX_NAME, H5_ITER_NATIVE,
                                hdf5_search_callback, void_ptr(&search_info));
#endif
...
inline hid_t load_and_convert_hdf5(eT *dest, hid_t dataset, hid_t datatype,
                                   uword n_elem) {
  ...
  if (is_equal) {
    ...
    hid_t status = arma_H5Dread(dataset, datatype, H5S_ALL, H5S_ALL,
                                H5P_DEFAULT, void_ptr(v.memptr()));
