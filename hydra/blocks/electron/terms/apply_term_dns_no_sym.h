...
void apply_term_dns_no_sym(IndexingIn &&indexing_in, IndexingOut &&indexing_out,
  ...
  for (bit_t dns_in : indexing_in.states_dns()) {
    if (non_zero_term(dns_in)) {
      auto [dns_out, coeff] = term_action(ups_in);
      idx_t idx_dns_out = indexing_out.index_dns(dns_out);

      idx_t idx_out_start = idx_dns_out;
      idx_t idx_out_end = idx_dns_out + indexing.size();

      for (idx_t idx_out = idx_out_start, idx_t idx_in = idx_dns_in;
           idx_out < idx_out_end;
           idx_out += size_ups_out, idx_in += size_ups_in) {
