...
inline bool close(double const &x, double const &y,
                  real_t<double> atol = hydra::atol<double>::val(),
                  real_t<double> rtol = hydra::rtol<double>::val()) {
  return (std::abs(x - y) <= (atol + rtol * std::abs(y)));
}

inline bool close(complex const &x, complex const &y,
                  real_t<complex> atol = hydra::atol<complex>::val(),
                  real_t<complex> rtol = hydra::rtol<complex>::val()) {
  return (std::abs(x - y) <= (atol + rtol * std::abs(y)));
}
...
inline bool close(arma::Col<coeff_t> const &vec1,
                  ...
                  real_t<coeff_t> atol = hydra::atol<coeff_t>::val(),
                  real_t<coeff_t> rtol = hydra::rtol<coeff_t>::val()) {
