...
State::State(block_t const &block, arma::Col<coeff_t> const &vector)
    : real_(hydra::isreal<coeff_t>()), n_rows_(block.size()), n_cols_(1),
      block_(block) {
...
State State::col(int64_t n, bool copy) const try {
  ...
  HydraRethrow("Unable to retrieve column State");
  return State();
}
...
arma::vec State::vector(int64_t n, bool copy) const try {
  if (!real_) {
    ...
    HydraThrow(std::invalid_argument, "Negative column index");
  }
  return arma::vec(storage_.data() + n * n_rows_, n_rows_, copy, !copy);
} catch (...) {
  HydraRethrow("Unable to access real vector of State");
  return arma::vec();
}
...
arma::mat State::matrix(bool copy) const try {
  ...
  HydraRethrow("Unable to access real matrix of State");
  return arma::mat();
}
...
arma::cx_vec State::vectorC(int64_t n, bool copy) const try {
  ...
  HydraRethrow("Unable to access complex vector of State");
  return arma::cx_vec();
}
...
arma::cx_mat State::matrixC(bool copy) const try {
  ...
  HydraRethrow("Unable to access complex matrix of State");
  return arma::cx_mat();
}
