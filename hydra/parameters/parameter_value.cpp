...
parameter_value::parameter_value(const string& s)
  : the_string(s),
    the_type(is_string)
{
}
...
parameter_value::parameter_value(const complex<double>& d)
  : valcomplex(d),
    the_type(is_complex)
{
}
    ...
    the_type(p.the_type)
{
  switch(the_type)
    ...
    default:
      error (domain_error("illegal parameter type in parameter_value::parameter_value"));
    }
}
...
parameter_value& parameter_value::operator=(const parameter_value& p)
{
  ...
  switch(the_type)
    ...
    default:
      error (domain_error("illegal parameter type in parameter_value::operator="));
    }
...
parameter_value& parameter_value::operator=(const string& s)
{
  the_type=is_string;
  the_string=s;
  return *this;
...
parameter_value& parameter_value::operator=(const complex<double>& c)
{
  the_type=is_complex;
  valcomplex=c;
  return *this;
...
parameter_value::operator const string&() const
{
  if(the_type!=is_string)
    type_error(is_string);
...
parameter_value::operator complex<double>() const
{
  if(the_type==is_complex)
    return valcomplex;
  else if(the_type==is_float)
    return complex<double>(valfloat,0.);
  else if(the_type==is_integer)
    return complex<double>(valint,0.);
  else if(the_type==is_bool)
    return complex<double>(double(valbool),0.);
  else
...
void parameter_value::type_error(parameter_value_type expected) const
  {
    string text = "Parameter should be ";
    switch (expected)
    {
    case is_integer:
      text += "integer";
    ...
    default:
      error (domain_error("illegal parameter type in parameter_value::type_error"));
    }
  ...
  error(parameter_type_error(text));
}
...
ostream& operator<< (ostream& out, const parameter_value& val)
{
  switch(val.type())
    {
    case is_integer:
...
//      break;
    ...
    default:
      error ( domain_error("illegal parameter type in operator<<(ostream&,const parameter_value&)") );
    }
