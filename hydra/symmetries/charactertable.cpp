...
namespace hydra { namespace symmetries {
    ...
    std::vector<std::string> CharacterTable::names() const
    { return names_; }
    ...
    complex CharacterTable::character(const std::string& name, const int& n_sym) const
      ...
      for(int i = 0; i < nreps; ++i)
	{
	  ...
	  int n_allowed_ops;	
	  if(pos!=std::string::npos) 
	    n_allowed_ops=atoi(tobeparsed.substr(pos+1,std::string::npos).c_str());
	  else 
	    n_allowed_ops=-1;
	  ...
	  for(int so = 0; so < n_allowed_ops; ++so)
	    {
	      ...
	      if(!File.good())
		{ 
		  std::cerr << "Read Error in read_charactertable (I)"
			    << std::endl;
		  exit(EXIT_FAILURE);
		}
	  ...
	  if(!File.good())
	    { 
	      std::cerr << "Read Error in read_charactertable (II)"
			<< std::endl;
	      exit(EXIT_FAILURE);
	    }
	  ...
	  for(int so=0; so < n_allowed_ops; ++so)
	    {
	      ...
	      if(!File.good()) 
		{ 
		  std::cerr << "Read Error in read_charactertable (III)"
			    << std::endl;
		  exit(EXIT_FAILURE);
		}
      ...
      return CharacterTable(space_group, names, 
			    allowed_symmetries_arr, characters_arr);
    }
