#ifndef HYDRA_APPLICATIONS_TJFTLM_OPTIONS_H_
#define HYDRA_APPLICATIONS_TJFTLM_OPTIONS_H_
#include <string>
#include <mpi.h>
#include "clara.hpp"
...
void parse_cmdline(std::string& outfile, std::string& latticefile, std::string& couplingfile, std::string& corrfile, std::string& corrcouplingfile, int& nup, int& ndown, int& iters, int& verbosity, int& seed, bool& kinetic, int& argc, char** argv)
  {
    int mpi_rank;
    ...
    auto parser =
      ...
      clara::Opt(corrcouplingfile, "corrcouplingfile")["-c"]["--corrcouplingfile"]("name of file describing the current operator couplilngs") |
      clara::Opt(method, "method")["-m"]["--method"]("method to use (FTLM, Sym0, SymR)") |
  ...
  if( !cmd_args ) 
      ...
      if ((verbosity >= 1) && (mpi_rank == 0))
	{
	  std::cout <<
	    ...
	    "corrcouplingfile: " << corrcouplingfile << std::endl <<
