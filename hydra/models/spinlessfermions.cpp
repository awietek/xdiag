...
namespace hydra { namespace models {
    ...
    lila::Matrix<double> SpinlessFermions::matrix(double t, double V, int qn) const
    {
      ...
      for (auto pair : interactions_)
	{
	  ...
	  for (auto state : hs)
	    {  
	      hamilton(idx, idx) += V * ((double)gbit(state, s1)) * 
		((double)gbit(state, s2));
	      ++idx;
	    }
	}
    ...
    lila::Matrix<complex> SpinlessFermions::matrix
    (double t, double V, int qn, CharacterTable& character_table, 
	  ...
	  for (int idx : range<>(indexing.size()))
		  ...
		  ((state & flipmask) != flipmask))
		{
		  ...
		  int n_sym = indexing.find_representative(&representative);
		      ...
		      double fermi = 
			popcnt(gbits(state, s2-s1-1, s1+1)) % 2==0 ? 1. : -1.;
		      double fermi2 = space_group.fermi_sign<Spinhalf<uint64>>
			  (n_sym, new_state);			
