...
IndexingSymmetricFermionic<bit_t, GroupAction>::IndexingSymmetricFermionic(
  ...
  utils::check_nup_spinhalf(n_sites, n_up, "IndexingSymmetricFermionic");
    ...
    if (rep != state) {
      ...
      if (std::abs(norm) > 1e-6) {
        norm_of_raw_state_[idx] =
            fermi_sign_of_permutation(state, sym_ptr[rep_sym * n_sites],
                                      fermi_work.data()) *
...
template class IndexingSymmetricFermionic<uint16, PermutationGroupLookup<uint16>>;
template class IndexingSymmetricFermionic<uint32, PermutationGroupLookup<uint32>>;
template class IndexingSymmetricFermionic<uint64, PermutationGroupLookup<uint64>>;
