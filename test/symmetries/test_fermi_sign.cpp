...
template <class bit_t> void test_fermi_sign(int n_sites) {
  ...
  for (auto state : Subsets<bit_t>(n_sites)) {
    for (int sym = 0; sym < n_sites; ++sym) {
      ...
      auto fermi1 = symmetries::fermi_sign_of_permutation(state, permutation_array.data()+sym*n_sites, work.data());
      auto fermi2 = symmetries::fermi_sign_of_permutation_sort(state, permutation_array.data()+sym*n_sites, work.data());
