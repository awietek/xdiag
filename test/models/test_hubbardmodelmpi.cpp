...
#include "../entanglement/testcases_entanglement_entropy.h"

using namespace hydra;
using namespace lila;

template <class coeff_t>
void test_hubbardmodelmpi(BondList bondlist, Couplings couplings) {
  int mpi_rank;
  ...
  for (int nup = 0; nup <= n_sites; ++nup)
    for (int ndn = 0; ndn <= n_sites; ++ndn) {
      ...
      // Run Lanczos
      auto res = LanczosEigenvalues(multiply_H, startstate, 1e-12, 0, "Ritz");
...
TEST_CASE("HubbardModelMPI", "[HubbardModel]") {
  ...
  for (int i = 0; i < 20; ++i) {
    ...
    couplings["U"] = U;
    if (mpi_rank == 0)
      printf("HubbardModelMPI: two-site exact solution test, U=%f\n", U);
  ...
  if (mpi_rank == 0)
    printf("HubbardModelMPI: Heisenberg triangle test, N=3\n");
  ...
  ///////////////////////////////////////////////////
  // Test random all to all, with J
  for (int n_sites = 5; n_sites < 6; ++n_sites) {
    if (mpi_rank == 0) {
      printf("HubbardModelMPI: interacting all-to-all test (cplx), ");
      printf("N=%d\n", n_sites);
    }
    std::tie(bondlist, couplings) = entanglementtestcases::tJrandom_model(n_sites);
    couplings["U"] = 1.23;
    test_hubbardmodelmpi<complex>(bondlist, couplings);
  }
  ...
  if (mpi_rank == 0)
    printf("HubbardModelMPI: MATLAB full spectrum, chain N=4, no Hubbard U\n");
  std::tie(bondlist, couplings) = randomAlltoAll4NoU();
  test_hubbardmodelmpi<double>(bondlist, couplings);

  if (mpi_rank == 0)
    printf("HubbardModelMPI: MATLAB full spectrum, chain N=4, Hubbard U\n");
