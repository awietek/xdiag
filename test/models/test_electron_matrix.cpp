...
TEST_CASE("electron_matrix", "[models]") {
  ...
  auto H1 = matrix(bondlist, couplings, block, block);
  ...
  REQUIRE(lila::close(Real(H1), H1_correct));
  for (int i = 0; i < 24; ++i)
    for (int j = 0; j < 24; ++j) {
  ...
  for (int i = 0; i < 20; ++i) {
    ...
    auto H = matrix(bondlist, couplings, block2, block2);
  ...
  for (int n_sites = 3; n_sites < 8; ++n_sites) {
    ...
    for (int nup = 0; nup <= n_sites; ++nup)
      for (int ndn = 0; ndn <= n_sites; ++ndn) {
        ...
        auto H = matrix(bondlist, couplings, block3, block3);
        auto evecs = lila::EigenvaluesSym(H);
        double e0 = evecs(0);
