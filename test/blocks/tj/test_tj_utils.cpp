      ...
      for (int ndn = 0; ndn <= N - nup; ++ndn) {
        ...
        for (auto up : Combinations(N, nup))
          for (auto dnc : Combinations(N - nup, ndn)) {
            ...
            REQUIRE(up == up2);
            REQUIRE(dnc == dnc2);
            // Log.out("up : {} dnc: {}", bits_to_string(up, N),
            // bits_to_string(dnc,N)); Log.out("upc: {} dn : {}",
        ...
        for (auto holes : Combinations(N, n_holes))
          for (auto spins : Combinations(charge, nup)) {
            ...
            // Log.out("holes: {} spins: {}", bits_to_string(holes, N),
            // bits_to_string(spins,charge)); Log.out("up   : {} dn   :
	    ...
	    if (has_double_occupations(up, dn))
	      continue;
            ...
            // Log.out("up   : {} dn   : {}", bits_to_string(up, N),
            //               bits_to_string(dn, N));
            // Log.out("holes: {} spins: {}", bits_to_string(holes, N),
