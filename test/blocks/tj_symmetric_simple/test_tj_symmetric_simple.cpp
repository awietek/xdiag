...
template <class bit_t> void test_indices(tJSymmetricSimple<bit_t> const &tj) {
...
void test_representative_character(tJSymmetricSimple<bit_t> const &tj) {
  ...
  for (int nup = 0; nup <= n_sites; ++nup) {
    for (int ndn = 0; ndn <= n_sites; ++ndn) {
      ...
      for (int k = 0; k < n_sites; ++k) {
        ...
        auto tj = tJSymmetricSimple<bit_t>(n_sites, nup, ndn, space_group, irrep);
...
TEST_CASE("tJSymmetricSimple", "[models]") {
  ...
  lila::Log.out("tJSymmetricSimple block test: Triangular 3x3");
  int n_sites = 9;
