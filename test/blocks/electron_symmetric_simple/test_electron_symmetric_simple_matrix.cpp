...
void test_symmetric_spectra_simple(BondList bondlist, Couplings couplings,
...
template <class bit_t>
void test_hubbard_symmetric_spectrum_chains_simple(int n_sites) {
  ...
  test_symmetric_spectra_simple<bit_t>(bondlist, couplings, space_group, irreps,
                                       multiplicities);
  ...
  test_symmetric_spectra_simple<bit_t>(bondlist_hb, couplings_hb, space_group,
                                       irreps, multiplicities);
}

TEST_CASE("ElectronSymmetricSimple_Matrix",
          "[models][ElectronSymmetricSimple]") {
  using namespace hydra::testcases::electron;
  ...
  for (int k = 0; k < (int)irreps.size(); ++k) {
    auto irrep = irreps[k];
    auto electron =
        ElectronSymmetricSimple<uint16_t>(n_sites, nup, ndn, space_group, irrep);
    auto H_sym = MatrixCplx(bondlist, couplings, electron, electron);
    complex U2 = 2 * U;
    complex UU = U;
    complex tp = t;
  ...
  for (int n_sites = 2; n_sites < 7; ++n_sites) {
    test_hubbard_symmetric_spectrum_chains_simple<uint16_t>(n_sites);
    test_hubbard_symmetric_spectrum_chains_simple<uint32_t>(n_sites);
    test_hubbard_symmetric_spectrum_chains_simple<uint64_t>(n_sites);
  }
  ...
  couplings["J"] = 0.4;
  test_symmetric_spectra_simple<bit_t>(bondlist_hb, couplings, space_group,
                                       irreps, multiplicities);
  ...
  // test a 3x3 triangular lattice with complex hoppings
  {
    ...
    test_symmetric_spectra_simple<bit_t>(bondlist, couplings, space_group,
                                         irreps, multiplicities);
  }
}
