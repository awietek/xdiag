...
TEST_CASE("analytic_case_free_particle_1D", "[time_evolution]"){
    // some constants
    int n_sites = 17; int nup = 1; int ndn = 0;
    double t = 1.5;
    ...
    for(auto time: times){
        double tol = 1e-2;
        auto w_expokit = zahexpv(time, bonds, psi_0, tol, 4);
...
TEST_CASE("analytic_case_free_particle_2D", "[time_evolution]"){
    ...
    int n_sites = L*L ; int nup = 1; int ndn = 0;
    double t = 1.5; double t1 = 0.3;
    const double pi = 3.14159265358979323846;
    const cx_double i = {0, 1};
            ...
            for (int n = 0; n < L; n++) {
                ...
                for (int k1 = 0; k1 < L; k1++) {
                    for (int k2 = 0; k2 < L; k2++) {
                        ...
                        energy += -2 * t1 * (cos(2 * pi * (k1 + k2) / L) + cos(2 * pi * (k1 - k2) / L));

                        //exponent
                        cx_double a_k1k2 = -i * time * energy + 2 * pi * i * (k1 * m + k2 * n) / L;
                        c_mn += exp(a_k1k2);
                    }
                }
                int s = L * m + n;
