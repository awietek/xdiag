...
Computes the `neigs` algebraically smallest eigenvalues **and** the corresponding eigenvectors of a hermitian operator using the [LOBPCG](eigs_lobpcg.md) block eigensolver. In contrast to the [Lanczos](eigs_lanczos.md) algorithm, LOBPCG reliably resolves excited states and their degeneracies, at the cost of a higher memory footprint. If only the eigenvalues are needed, use [eigvals](eigvals.md).
...
=== "C++"
	...
	eigs(OpSum const &ops, Block const &block, int64_t neigs,
...
=== "Julia"
	...
	ops["J"] = 1.0
	eigenvalues, eigenvectors = eigs(ops, block, 3)
	```
=== "C++"
	```c++
	...
	ops["J"] = 1.0;
	auto [eigenvalues, eigenvectors] = eigs(ops, block, 3);
	```
